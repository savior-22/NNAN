...
def build(total_batch):
    ...
    with tf.variable_scope('attention_module') as scope:
        W = tf.ones(FLAGS.input_size)
        edge_emb = tf.matmul(X, tf.diag(W))
        ...
        for i in range(FLAGS.input_size):
            attention_FC = tf.matmul(edge_emb, A_W_unstack[i]) + A_b_unstack[i]
    ...
    with tf.variable_scope("learning_module") as scope:
        ...
        average_L_FC = tf.nn.relu(tf.matmul(G, L_W1) + L_b1)
        average_O = tf.matmul(average_L_FC, L_W2) + L_b2
    ...
    with tf.name_scope("Loss") as scope:
        ...
        prediction = tf.reduce_max(average_O, 1)
        correct_prediction = tf.equal(tf.argmax(average_O, 1),
                                  tf.argmax(Y, 1))

        test_label = Y[:, 0]
        test_data = average_O[:, 0]
    ...
    for op in [loss, test_data, test_label]:
