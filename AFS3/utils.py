...
def cross_validation1(intMat, seeds, A_sim, B_sim, num=5):
    ...
    for seed in seeds:
        ...
        for i in range(num):
            ...
            test_data = np.vstack((np.array([[k/num_drugs, k % num_drugs] for k in pos_ii], dtype=np.int32),np.array([[k/num_drugs, k % num_drugs] for k in neg_ii_new], dtype=np.int32)))
            train_data = np.vstack((np.array([[k/num_drugs, k % num_drugs] for k in pos_diff_set], dtype=np.int32), np.array([[k/num_drugs, k % num_drugs] for k in neg_diff_set], dtype=np.int32)))

            edge_encodeA = embedding(intMat * W, A_sim)
            edge_encodeB = embedding((intMat * W).T, B_sim)
            ...
            train_label0 = intMat[train_x, train_y]
            train_label1 = 1 - intMat[train_x, train_y]
...
def cross_validation23(intMat, seeds, sim, num=10):
    ...
    for seed in seeds:
        ...
        for i in range(10):
            ...
            test_x, test_y = test_data[:, 0], test_data[:, 1]
            A = delete(intMat, train_data, seed, 0)
            A[test_x, test_y] = 0
...
def write(x, y, pred):
    with open('pred.txt', 'a+', encoding='utf-8') as f:
        for i in range(len(x)):
            text = str(x[i]) + "\t" + str(y[i]) + "\t" + str(pred[i]) + "\n"
            f.write(text)
